...
static unsigned long wasted_clb_v5, wasted_bram_v5, wasted_dsp_v5;
...
static vector<unsigned long> clb_req (MAX_SLOTS);
static vector<unsigned long> bram_req(MAX_SLOTS);
static vector<unsigned long> dsp_req(MAX_SLOTS);

static vector <vector <unsigned long>> conn_matrix = vector <vector<unsigned long>> (MAX_SLOTS, vector<unsigned long> (MAX_SLOTS, 0));;
static unsigned long num_conn_slots;

static Vecpos fs(MAX_SLOTS);

int solve_milp_virtex_5(param_from_solver *to_sim)
{
    unsigned long status, i ,k, j, l, s, m;
...
//#endif
        ...
        ******************************************************************/
        for(i =0; i < num_slots; i++) {
            ...
            for(k = 0; k < 2; k++) {
                ...
                model.addConstr(exp <= (l + 1) / 2, "49000");
      ...
      **********************************************************************/
        for(i = 0; i < num_slots; i++) {
            ...
            model.addConstr(clb_per_tile * exp_res >= clb_req[i],"68");
            model.addConstr(wasted[i][0] == (clb_per_tile * exp_res) - clb_req[i],"168"); //wasted clbs

            model.addConstr(bram_per_tile * exp_bram >= bram_req[i],"69");
            model.addConstr(wasted[i][1] == bram_per_tile * exp_bram - bram_req[i],"169");

            model.addConstr(dsp_per_tile * exp_dsp >= dsp_req[i],"70");
            model.addConstr(wasted[i][2] == (dsp_per_tile * exp_dsp) - dsp_req[i], "170");
        }
...
*/
        ...
        for(i = 0; i < num_conn_slots; i++) {
                ...
                dist_1 = conn_matrix[i][1] - 1;
                dist_2 = conn_matrix[i][2];
            ...
            for(i = 0; i < num_slots; i++) {
                ...
                cout << i << "\t" << x[i][0].get(GRB_DoubleAttr_X) <<"\t"
                    ...
                    clb[i][1].get(GRB_DoubleAttr_X) << "\t" << (clb[i][1].get(GRB_DoubleAttr_X) -
                     clb[i][0].get(GRB_DoubleAttr_X)) * h[i].get(GRB_DoubleAttr_X) * clb_per_tile<< "\t" << clb_req[i]

                    <<"\t" << bram[i][0].get(GRB_DoubleAttr_X) <<"\t" <<
                    bram[i][1].get(GRB_DoubleAttr_X) << "\t" << (bram[i][1].get(GRB_DoubleAttr_X) -
                    bram[i][0].get(GRB_DoubleAttr_X)) * h[i].get(GRB_DoubleAttr_X) * bram_per_tile<< "\t" << bram_req[i]

                    << "\t" << dsp[i][0].get(GRB_DoubleAttr_X) << "\t" <<
                    dsp[i][1].get(GRB_DoubleAttr_X) << "\t" << (dsp[i][1].get(GRB_DoubleAttr_X) -
                            dsp[i][0].get(GRB_DoubleAttr_X)) * h[i].get(GRB_DoubleAttr_X) * dsp_per_tile << "\t" << dsp_req[i] <<endl;
